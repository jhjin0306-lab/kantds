...
8. 다음 질문에 답하세요.
   ...
   -> 아니오. 한 주문에 여러 상품이 포함되어 있으면 order_id가 여러 행으로 나타날 수 있다.
   
   Q3. 이번 조회에서 order_items가 중요한 연결 테이블인 이유는 무엇인가요?
   -> orders 테이블에서는 주문 정보가 들어있고,Products에는 상품 정보가 있지만
   -> 어떤 주문에 어떤 상품이 포함되어있는지는 order_items가 연결한다.
...
-- [코드 작성란]
--orders를 기준으로 customers를 customer_id로 JOIN하세요.
--2. orders와 order_items를 order_id로 JOIN하세요.
--3. order_items와 products를 product_id로 JOIN하세요.
--4. 다음 컬럼을 조회하세요.
--   - orders.order_id
...
select
o.order_id, o.order_date, c.customer_id, c.city, p.product_id, oi.qty, oi.price
from orders o 
join customers c 
...
order by order_id asc, p.product_id asc;
...
8. 다음 질문에 답하세요.
   ...
   -> left join 으로 주문이 없는 고객은 orders 컬럼이 null인 상태로 생성
   -> 그 후 where o.store_id = 1 조건을 적용하면
   -> null=1 인 행은 True가 아니라 UNKNOWN으로 True행만 넘기는 where에서 해당 고객이 제외
   Q2. ON에 o.store_id = 1을 작성하면 주문이 없는 고객이 왜 유지되나요?
   -> ON 조건은 어떤 orders 행을 customers와 연결할지를 결정
...
--order_id가 null인거 확인
select count(*) as no_store_1_order_count
from customers c 
left join orders o on c.customer_id = o.customer_id 
...
9. 다음 질문에 답하세요.
   ...
   -> 배송정보가 없는 주문 포함 모든 주문을 보여줘야 하기 때문에 LEFT JOIN 해야한다.
   
   Q2. shipment_id가 NULL인 행은 어떤 의미인가요?
   -> 주문은 존재하지만 배송정보는 없다는 의미이다.
   
   Q3. 조인 후 행 수가 orders보다 많아졌다면 어떤 관계나 데이터를 먼저 점검해야 하나요?
   -> 하나의 주문에 여러 배송정보가 있는지 확인한다. (1:N)
   
   Q4. 조인 결과 검증을 위해 행 수, 중복, NULL을 함께 확인해야 하는 이유는 무엇인가요?
   -> 행 수 : 데이터가 증가했거나 감소한게 있는지 확인
   -> 중복 : 1:N이나 중복 데이터가 있는지 확인
   -> NULL : 연결 안 된 데이터가 있는지 확인
   
   Q5. LEFT JOIN한 shipments의 status 조건을
       ON 절에 작성하는 경우와 WHERE 절에 작성하는 경우
       결과가 어떻게 달라질 수 있나요?
   -> ON : 주문 정보에 조건에 맞는 배송 정보가 붙음
   -> WHERE : where절 조건에 맞지 않는 배송정보는 결과에서 제거될 수 있음.
...
select count(*) as no_shipment_count
from orders o
join customers c  on c.customer_id =o.customer_id 
left join shipments s on o.order_id = s.order_id
...
group by o.order_id 
having count(*)>=2
order by o.order_id, row_count desc;
...
-> 최종 결과에 필요한 칼럼이 어느 테이블에 있는지
-> 테이블 간 연결 키 및 관계 유형 확인 필요.

2. 1:N 관계에서 행 수가 증가하는 이유는 무엇인가요?
-> 기준 테이블의 한 행에 상대 테이블의 여러 행이 연결될 수 있음
-> 기준 행이 매칭되는 개수만큼 여러 행으로 확장되기 때문

3. LEFT JOIN에서 오른쪽 테이블 조건을 ON과 WHERE에 둘 때 결과가 달라지는 이유는 무엇인가요?
-> ON은 어떤 행을 연결할 지 결정하는데
-> WHERE는 조인이 끝난 결과에서 어떤 행을 남길 지를 결정하기 때문

4. 조인 결과를 검증할 때 어떤 항목을 확인해야 하나요?
-> Join 전 후 행수
-> 기준 키의 중복 여부
-> left join 결과와 null 여부를 함께 확인
