...
9. 다음 질문에 답하세요.
   ...
   -> 첫번째 날 부터 현재 행까지의 daily_sales를 합산
   
   Q2. moving_avg_7d는 현재 행을 포함해 최대 몇 개 행을 평균내나요?
   -> 7개,6 PRECEDING 현재 행보다 앞의 6개행을 뜻하고 current row까지 포함하여 총 7개
...
7. 다음 질문에 답하세요.
   ...
   -> 날짜 순서 상 첫번쨰 행보다 이전에 존재하는 행이 없다(1월1일자일경우 - 시계열 데이터 상황많음)
   
   Q2. 마지막 날짜의 next_day_sales가 NULL인 이유는 무엇인가요?
   -> 마직막 행 이후에 참조할 다음 행이 없다.
   
   Q3. LAG와 LEAD에서 ORDER BY order_date가 중요한 이유는 무엇인가요?
   -> lag와 lead가 말하는 이전 행과 다음 행의 순서를 결정하기 때문
   -> 매출을 비교할 때에는 날씨 기준으로 이전·다음 행을 참조해야 정확한 전일·다음날 매출을 가져올 수 있다.
...
-- [코드 작성란]
--orders와 order_items를 이용해 날짜별 daily_sales를 계산하세요.
--2. daily_sales_summary CTE를 작성하세요.
--3. LAG(daily_sales, 1)을 이용하여 prev_day_sales를 계산하세요.
--4. LEAD(daily_sales, 1)을 이용하여 next_day_sales를 계산하세요.
--5. daily_sales - prev_day_sales 형태로
--   전일 대비 증감액 day_over_day_diff를 계산하세요.
--6. 결과를 order_date 오름차순
...
from daily_sales_summary)
select order_date, daily_sales, prev_day_sales, next_day_sales,
...
6. 전일 매출이 0인 경우 오류가 발생하지 않도록 NULLIF를 사용하세요. --  NULLIF(prev_day_sales, 0)
...
10. 다음 질문에 답하세요.
    ...
    -> 첫 날은 Lag로 참조할 이전 날이 없어서 NULL이 되는데 증감률도 NULL이 된다.
    Q2. NULLIF(prev_day_sales, 0)를 사용하는 이유는 무엇인가요?
    -> prev_day_sales의 값이 0으로 나오면  NULLI가 값을 NULL로 바꿔줘서 오류를 방지해줌
    Q3. day_over_day_pct가 음수라는 것은 비즈니스적으로 무엇을 의미하나요? -- daily_sales - prev_day_sales >0
    -> 전날보다 매출이 감소했다.
    Q4. 하루의 감소만으로 매출 추세가 악화되었다고 단정하기 어려운 이유는 무엇인가요?
    -> 하루치 매출 감소는 일시적일 수 있다. 누적합, 평균 등 다른 지표를 함께 참고해야 추세 파악이 가능할 것으로 보인다.
...
daily_sales - prev_day_sales as day_over_day_diff,
round((daily_sales - prev_day_sales )* 100 /  NULLIF(prev_day_sales, 0),2) as day_over_day_pct
from sales_report 
where daily_sales - prev_day_sales >0
order by order_date asc;
...
-> 전체 기간, 이동 흐름, 평균 이벤트 여부 등 다른 정보도 함께 확인 필요.
