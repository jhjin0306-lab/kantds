...
10. 다음 질문에 답하세요.
    ...
    -> initplan아래에 aggregate로도 나타날 수 있음 (서브쿼리가 별도로 먼저 계산되는 경우)
    
    Q2. orders와 order_items를 결합하는 작업은 어떤 Join 노드로 나타날 수 있나요?
    -> hash join, nested loop, merge join중 하나로 나타날 수 있음
    
    Q3. WindowAgg 이전에 Sort가 나타날 수 있는 이유는 무엇인가요?
    -> rank()가 partition by 고객_id, order by 부분이 있기 때문
    -> 순위 계산 전 필요한 순서로 데이터를 정렬하는 과정이 있음
...
read()over(partition by oi.customer_id order by oi.price desc)as price_rank
from  orders o 
...
-> 1. actual time이 상대적으로 큰 노드
-> 2. 처리 행 수가 많은 노드
-> 3. Row Removed by Filter가 많은 노드
-> 4. Sort와 같이 추가 비용이 발생하는 노드

6. 다음 질문에 답하세요.
   Q1. cost와 actual time은 어떤 차이가 있나요?
   -> cost는 옵티마이저가 계산한 상대적 비용
   -> actual time은 expain analyze로 실제 쿼리를 실행했을 때 실행된 실제 측정 시간
...
order by order_date desc;
--  ->  Seq Scan on orders  
--(cost=0.00..6411.00 rows=75236 width=20) (actual time=0.014..8.332 rows=75023.00 loops=1)
--        Filter: ((order_date >= '2023-01-01'::date) AND (order_date < '2024-01-01'::date))
--        Rows Removed by Filter: 224977
--        Buffers: shared hit=1911
--Planning Time: 0.062 ms
--Execution Time: 23.436 ms
...
6. 실행계획에서 다음 항목을 확인하세요.
   ...
   - Join 방식 :  Hash Join
   - Sort : Sort Key: o.order_date DESC
   - estimated rows : rows=75236
   - actual rows : rows=75023.00
   - Execution Time : 45.192 ms
...
9. 다음 질문에 답하세요.
   ...
   -> 스캔 방식, actual rows, actual time, row removed by filter 등
   
   Q2. Join 노드에서는 무엇을 확인해야 하나요?
   -> join 방식(hash/merge/nested loop), 결합조건, 결과 행 수와 시간 등
   
   Q3. Sort 노드에서는 무엇을 확인해야 하나요?
   -> Sort Key, 정렬방식, 정렬 대상 행 수 등
   
   Q4. Seq Scan이 나타났다고 해서 무조건 잘못된 실행계획이라고 할 수 있나요?
   -> 아니요. 테이블이 작거나 조회할 행 수가 전체에서 큰 비중을 차지할 때는 옵티마이저가 Seq Scan을
   -> 선택하는 것이 오히려 효율적일 수 있다.
...
--Sort  (cost=14996.29..15184.38 rows=75236 width=18) 
--(actual time=40.316..43.132 rows=75023.00 loops=1)
--  Sort Key: o.order_date DESC
--  Sort Method: external merge  Disk: 2136kB
--  Buffers: shared hit=2243, temp read=267 written=268
--  ->  Hash Join  (cost=1457.00..8902.49 rows=75236 width=18) 
--  (actual time=4.945..24.964 rows=75023.00 loops=1)
--        Hash Cond: (o.customer_id = c.customer_id)
--        Buffers: shared hit=2243
--        ->  Seq Scan on orders o  (cost=0.00..6411.00 rows=75236 width=12) 
--        (actual time=0.019..10.106 rows=75023.00 loops=1)
--              Filter: ((order_date >= '2023-01-01'::date) AND (order_date < '2024-01-01'::date))
--              Rows Removed by Filter: 224977
--              Buffers: shared hit=1911
--        ->  Hash  (cost=832.00..832.00 rows=50000 width=10) 
--        (actual time=4.757..4.758 rows=50000.00 loops=1)
...
--              (actual time=0.008..1.920 rows=50000.00 loops=1)
--                    Buffers: shared hit=332
...
-> 가장 아래 Scan부터 읽으면 데이터의 처리 과정을 이해하기 쉬움
 
2. Scan, Join, Sort, WindowAgg는 각각 어떤 작업을 의미하나요?
-> Scan : 데이터를 읽는 작업(index, seq등)
-> Join : 데이터의 결합
-> Sort : 지정된 기준으로 정렬
-> WindowAgg : 윈도우 함수 계산
